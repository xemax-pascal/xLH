...
class Xbox360Controller:
    ...
    def __init__(self, index=0, axis_threshold=0.2, raw_mode=False, event_timeout=1.0):
        ...
        if raw_mode:
            ...
            self.button_dpad_up = Button('button_dpad_up')
            self.button_dpad_down = Button('button_dpad_down')
            self.button_dpad_left = Button('button_dpad_left')
            self.button_dpad_right = Button('button_dpad_right')
            self.buttons = [
                self.button_a,
                self.button_b,
                self.button_x,
    ...
    def process_event(self, event):
        # print(event.number, event.value, event.type)
        ...
        if event.type == JS_EVENT_AXIS:
            if self.raw_mode:
                ...
                if num == 6:
                    ...
                    if val < 0:
                        self.button_dpad_left.when_pressed(self.button_dpad_left)
                    if val > 0:
                        self.button_dpad_right.when_pressed(self.button_dpad_right)
                    if val == 0:
                        self.button_dpad_left.when_released(self.button_dpad_left)
                        self.button_dpad_right.when_released(self.button_dpad_right)
                if num == 7:
                    # self.hat._value_y = int(val * -1)
                    if val < 0:
                        self.button_dpad_up.when_pressed(self.button_dpad_up)
                    if val > 0:
                        self.button_dpad_down.when_pressed(self.button_dpad_down)
                    if val == 0:
                        self.button_dpad_up.when_released(self.button_dpad_up)
                        self.button_dpad_down.when_released(self.button_dpad_down)
